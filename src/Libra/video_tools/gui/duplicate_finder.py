...
from PySide6.QtWidgets import (
    ...
    QFrame, QSplitter, QFileDialog, QMessageBox, QProgressBar, QHeaderView
)
...
VIDEO_EXTENSIONS = {'.mp4', '.mov', '.avi', '.mkv', '.m4v', '.mts', '.m2ts', '.wmv', '.flv', '.webm', '.3gp', '.mpg'}
    ...
    def get_video_metadata(filepath):
        ...
        try:
            cmd = [
                ...
                '-show_entries', 'stream=codec_name,width,height,r_frame_rate,tags',
                '-of', 'json', filepath
            ]
            # 15s timeout
            ...
            return {
                ...
                'codec': 'error', 'framerate': 0, 'resolution': 'Unknown',
                'make': 'Unknown', 'model': 'Unknown', 'has_gps': False
            }
...
class ScanWorker(QThread):
    ...
    def run(self):
        ...
        for f, d in data.items():
            if d.get('md5'): md5_map[d['md5']].append(f)
...
class VideoManagerApp(QMainWindow):
    ...
    def create_right_panel(self):
        ...
        fields = [
            "Filename", "Path", "Resolution", "Framerate", "Duration", "Size",
            "Codec", "Make", "Model", "GPS", "Status"
        ]
    ...
    def start_scan(self, folder_path):
        ...
        self.scan_worker.progress.connect(lambda c, t: (self.progress_bar.setMaximum(t), self.progress_bar.setValue(c)))
    ...
    def apply_filters(self):
        ...
        for row in range(root_count):
            ...
            if res_4k or res_1080 or res_720:
                match_res = False
                if res_4k and is_4k: match_res = True
                if res_1080 and is_1080: match_res = True
                if res_720 and is_720: match_res = True
                if not match_res: visible = False
            
            # Device Filters (AND logic)
            if visible and dev_apple:
                if "apple" not in str(info.get('make', '')).lower(): visible = False
            
            if visible and dev_iphone:
                if "iphone" not in str(info.get('model', '')).lower(): visible = False
            
            # Content Filters
            if visible and con_gps:
                if not info.get('has_gps'): visible = False
            ...
            if visible and con_dup:
                if status != "Exact Duplicate": visible = False
            
            if visible and con_sim:
                if status != "Similar Content": visible = False
    ...
    def update_inspector(self, item):
        ...
        self.inspector_labels["Duration"].setText(BackendUtils.format_duration(info.get('duration', 0)))
    ...
    # --- EXPORT ---
    def export_csv(self):
        if not self.all_video_data: return
        path, _ = QFileDialog.getSaveFileName(self, "Export CSV", os.path.expanduser("~/Desktop/video_report.csv"), "CSV Files (*.csv)")
        if path:
            try:
                import csv
                with open(path, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(["Filename", "Resolution", "FPS", "Duration", "Size", "Make", "Model", "GPS", "Duplicate Status", "Path"])
                    # Export visible rows only? or all? Let's export all found.
                    for fp, info in self.all_video_data.items():
                        writer.writerow([
                            info.get('filename'), info.get('resolution'), info.get('framerate'),
                            BackendUtils.format_duration(info.get('duration')), BackendUtils.format_size(info.get('size')),
                            info.get('make'), info.get('model'), info.get('has_gps'),
                            self.dup_status.get(fp, "header"), fp
                        ])
    ...
    def export_json(self):
        ...
        if not self.exact_groups and not self.similar_groups: return
        path, _ = QFileDialog.getSaveFileName(self, "Export JSON", os.path.expanduser("~/Desktop/duplicates.json"), "JSON Files (*.json)")
        if path:
            try:
