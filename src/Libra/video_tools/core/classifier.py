...
def classify_video(file_path: str) -> Dict:
    ...
    if not metadata['success']:
        return {
            'resolution': 'SD', 'orientation': 'W', 'framerate_category': 30,
            'actual_fps': 0.0, 'width': 0, 'height': 0,
            'make': None, 'model': None, 'has_camera': False, 'has_gps': False,
...
def _get_video_metadata(file_path: str) -> Dict:
    ...
    try:
        ...
        return {
            'success': True, 'width': width, 'height': height, 'fps': fps,
            'rotation': rotation, 'make': make, 'model': model,
            'has_camera': has_camera, 'has_gps': has_gps
        }
