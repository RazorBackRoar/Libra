    ...
    return {
        on: (event, handler) => {
            ...
            if (!allowedEventNames.has(name))
                throw new Error(`Unknown plugin event: ${name}`);
            if (name === "pet:drop")
                requirePermission("pet:drop");
            if (state.eventSubscriptions.size >= eventSubscriptionsQuota)
                throw new Error("Plugin event subscription quota exceeded.");
            ...
            if (name === "config:changed") {
                ...
                const wrapped = guardCallback((payload) => {
                    if (Array.isArray(payload.files))
                        for (const file of payload.files) {
                            if (isRecord(file) && typeof file.fileId === "string")
                                state.pickedFiles.add(file.fileId);
                        }
                    handler(payload);
                });
        ...
        off: (subscriptionId) => { state.eventSubscriptions.get(String(subscriptionId))?.(); state.eventSubscriptions.delete(String(subscriptionId)); },
    };
}
function opaqueId(prefix) { return `${prefix}-${Math.random().toString(36).slice(2)}-${Date.now().toString(36)}`; }
function isRecord(value) { return typeof value === "object" && value !== null && !Array.isArray(value); }
//# sourceMappingURL=plugin-sdk-events.js.map