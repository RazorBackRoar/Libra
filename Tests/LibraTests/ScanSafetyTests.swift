...
final class ScanSafetyTests: XCTestCase {
    func testWarning_skipsOfficialTestFolder() {
        let path = "/tmp/MetaBurn & Libra Test/videos"
        XCTAssertNil(ScanSafety.warning(for: [path]))
        let legacyPath = "/tmp/MetaBurn & L!bra Test/videos"
    ...
    func testFileCountWarning_thresholdAndTestFolder() {
        ...
        XCTAssertNil(
            ScanSafety.fileCountWarning(
                count: 800,
                paths: ["/tmp/MetaBurn & Libra Test/videos"]
            ))
    }

    func testDestinationInsideSource() {
        XCTAssertTrue(
            ScanSafety.destinationIsInsideSource(dest: "/Trip/Photos", sourceRoot: "/Trip"))
