...
/** Collect all matching file paths under root (recursive). */
async function collectFiles(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await collectFiles(full, extensionSet, out);
...
export async function scan(opts: {
  ...
  for (const p of paths) {
    if (cancelFlag.cancelled) return { files: [], cancelled: true };
    await collectFiles(p, extensionSet, filePaths);
  }
  ...
  return { files: results, cancelled: cancelFlag.cancelled };
}
