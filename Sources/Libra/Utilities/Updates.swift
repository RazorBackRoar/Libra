...
final class Updates {
    ...
    private static let userAgent = "libra-update-checker/1.0"
    ...
    static func checkForUpdates(currentVersion: String) async -> UpdateResult {
        ...
        guard
            let url = URL(
                string:
                    "https://api.github.com/repos/\(Brand.githubOrg)/\(Brand.githubRepo)/releases/latest"
            )
        else {
            return UpdateResult(
        ...
        do {
            ...
            let tag = (json["tag_name"] as? String ?? "").replacingOccurrences(
            ...
            writeCache(
                latestVersion: tag, downloadURL: htmlURL, releaseNotes: body,
    ...
    private static func writeCache(
        latestVersion: String, downloadURL: String?, releaseNotes: String?, releaseDate: String?
    ) {
