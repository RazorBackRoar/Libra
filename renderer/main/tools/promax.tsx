...
export function ProMax() {
  ...
  const handlePaths = useCallback(
    (paths: string[]) => {
      console.log("[promax:paths]", { count: paths.length });
  ...
  const scanMutation = useMutation({
    mutationFn: async () => {
      ...
      setProgress(null);
      setOpResults([]);
      console.log("[promax:scan:start]", { jobId, paths: session.droppedPaths });
    ...
    onSuccess: (result) => {
      ...
      console.log("[promax:scan:done]", { count: result.files.length });
    },
    onError: (err) => {
      setIsScanning(false);
      setProgress(null);
      console.log("[promax:scan:error]", { err });
  ...
  const handleCancel = async () => {
    ...
    console.log("[promax:cancel]", { jobId });
  ...
  const sortMutation = useMutation({
    mutationFn: async () => {
      console.log("[promax:sort:apply]", { mode: "ProMax", dryRun });
    ...
    onSuccess: ({ results }) => {
      ...
      if (dryRun) {
        ...
        toast.success("Sort applied", { description: `${results.length} files sorted into resolution + orientation folders.` });
      }
      console.log("[promax:sort:done]", { count: results.length, errors: errors.length });
    },
    onError: (err) => {
      console.log("[promax:sort:error]", { err });
  ...
  return (
    <ToolPage title="ProMax" category="organize">
      <Text variant="regular" color="secondary">
        Sort videos into resolution + orientation subfolders (e.g. 4K/landscape, 1080p/portrait).
      </Text>
      ...
      {hasFiles && (
        <>
          ...
          <div className="flex items-center gap-4 pt-2 border-t border-separator">
            ...
            >
              <PlayIcon className="size-4" />
              {sortMutation.isPending ? "Sorting…" : "Sort ProMax"}
            </Button>
