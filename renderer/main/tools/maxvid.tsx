...
export function MaxVid() {
  ...
  const handlePaths = useCallback(
    (paths: string[]) => {
      console.log("[maxvid:paths]", { count: paths.length });
  ...
  const scanMutation = useMutation({
    mutationFn: async () => {
      ...
      setProgress(null);
      setOpResults([]);
      console.log("[maxvid:scan:start]", { jobId, paths: session.droppedPaths });
    ...
    onSuccess: (result) => {
      ...
      console.log("[maxvid:scan:done]", { count: result.files.length });
    },
    onError: (err) => {
      setIsScanning(false);
      setProgress(null);
      console.log("[maxvid:scan:error]", { err });
  ...
  const handleCancel = async () => {
    ...
    console.log("[maxvid:cancel]", { jobId });
  ...
  const sortMutation = useMutation({
    mutationFn: async () => {
      console.log("[maxvid:sort:apply]", { mode: "MaxVid", dryRun });
    ...
    onSuccess: ({ results }) => {
      ...
      if (dryRun) {
        ...
        toast.success("Sort applied", { description: `${results.length} files sorted into resolution + orientation + FPS folders.` });
      }
      console.log("[maxvid:sort:done]", { count: results.length, errors: errors.length });
    },
    onError: (err) => {
      console.log("[maxvid:sort:error]", { err });
  ...
  const countPills: CountPill[] = [
    ...
    { label: "Landscape", count: session.scannedFiles.filter((f) => f.orientation === "landscape").length, color: "blue" },
    { label: "Portrait", count: session.scannedFiles.filter((f) => f.orientation === "portrait").length, color: "purple" },
  ];
  ...
  return (
    <ToolPage title="MaxVid" category="organize">
      <Text variant="regular" color="secondary">
        Sort videos into resolution + orientation + FPS subfolders for maximum granularity.
      </Text>
      ...
      {hasFiles && (
        <>
          ...
          <div className="flex items-center gap-4 pt-2 border-t border-separator">
            ...
            >
              <PlayIcon className="size-4" />
              {sortMutation.isPending ? "Sorting…" : "Sort MaxVid"}
            </Button>
