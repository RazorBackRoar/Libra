...
export function VidRes() {
  ...
  const handlePaths = useCallback(
    (paths: string[]) => {
      console.log("[vidres:paths]", { count: paths.length });
  ...
  const scanMutation = useMutation({
    mutationFn: async () => {
      ...
      setProgress(null);
      setOpResults([]);
      console.log("[vidres:scan:start]", { jobId, paths: session.droppedPaths });
    ...
    onSuccess: (result) => {
      ...
      console.log("[vidres:scan:done]", { count: result.files.length });
    },
    onError: (err) => {
      setIsScanning(false);
      setProgress(null);
      console.log("[vidres:scan:error]", { err });
  ...
  const handleCancel = async () => {
    ...
    console.log("[vidres:cancel]", { jobId });
  ...
  const sortMutation = useMutation({
    mutationFn: async () => {
      console.log("[vidres:sort:apply]", { mode: "VidRes", dryRun });
    ...
    onSuccess: ({ results }) => {
      ...
      if (dryRun) {
        ...
        toast.success("Sort applied", { description: `${results.length} files moved into resolution folders.` });
      }
      console.log("[vidres:sort:done]", { count: results.length, errors: errors.length });
    },
    onError: (err) => {
      console.log("[vidres:sort:error]", { err });
  ...
  return (
    <ToolPage title="VidRes" category="organize">
      <Text variant="regular" color="secondary">
        Sort videos into resolution folders — 4K, 1080p, 720p, SD — under each file's own directory.
      </Text>
      ...
      {hasFiles && (
        <>
          ...
          <div className="flex items-center gap-4 pt-2 border-t border-separator">
            ...
            >
              <PlayIcon className="size-4" />
              {sortMutation.isPending ? "Sorting…" : "Sort by Resolution"}
            </Button>
